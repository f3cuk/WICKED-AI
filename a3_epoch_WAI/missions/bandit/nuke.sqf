...
if(isServer) then {
	...
	_fn_position	= [10,1] call find_position;
	_position		= _fn_position select 0;
	_missionType	= _fn_position select 1;
	
	[_mission,_position,"hard","Nuclear device","MainBandit",true] call mission_init;
	...
	_crate setPos [(_position select 0),(_position select 1),-20];

	//Nuke site
	//Create the scenery
	private["_tent","_device","_laptop","_berrels","_boxEmpty","_palletMil","_pallets","_solar","_gen","_hbar"];
	...
	_laptop attachTo [_table,[0,0,0.55]];
	_laptop setDir 180;
	...
	_baserunover = [_tent,_device,_laptop,_berrels,_boxEmpty,_palletMil,_pallets,_solar,_gen,_hbar];
	...
	_complete = [
		...
		"A soldier squad have set up a nuclear device! Hurry disable it before it goes off",	// mission announcement
		"Survivors have taken control of the nuclear device!",									// mission success
		"Nuclear device detonated!!"															// mission fail
	...
	if(_complete) then {
		...
		if(ai_crypto_gain) then {
			...
			diag_log format["ADMIN: Created crypto device for %1 with %2 at %3","NUKE",4000,_pos];
			_mps attachTo [_laptop,[-0.1,0,0]];
			_mps setVariable["Crypto",4000,true];
		};
	};
