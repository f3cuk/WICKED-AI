crate_add_loot = {
	private ["_uniq","_iItem","_iClass","_iPos","_radius","_item","_itemTypes","_index","_weights","_cntWeights","_qty","_max","_tQty","_canType","_mags","_dateNow","_magQty","_item2","_uniq"];
				...
				if ((_item2 != "") && (isClass(configFile >> "CfgWeapons" >> _item2))) then{
					...
					if ((count _mags) > 0) then{
						...
						if (!(_item2 in MeleeWeapons)) then{
							...
							if (_magQty > 3) then{
								_crate addMagazineCargoGlobal[(_mags select 0), (round(random 1) + 1)];
							};
						};
					};
				};
...
private ["_itemType","_itemTypes","_crate", "_loot_type", "_loot", "_chance", "_item", "_type", "_amount", "_mags"];
_crate = _this select 0;
_lootTable = _this select 1;

if (_lootTable == "Random") then {
	_lootTable = wai_loot_tables call BIS_fnc_selectRandom;
};
...
diag_log format["WAI: creating loot amount: %1 with loot table %2",_amount,_lootTable];
