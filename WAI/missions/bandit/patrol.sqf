if(isServer) then {
	...
	//Spawn units
	_units = [[_position select 0,_position select 1,0.1],3,"Hard","Random",3,"Random","bandit","Random","bandit",_mission] call spawn_group;
	...
	_complete = [
		...
		format["bandites have started to patrol in %1",(_location select 0)],	// mission announcement
		"Bandits have taken out the bandit patrol!",		// mission success
		"bandites managed to reach their destination"		// mission fail
