_this spawn {
	...
	// Allow the vehicle to take damage
	...
	[_vehicle,_unitgroup] spawn {
		...
		createGuardedPoint [east, [0,0], -1, _vehicle];
	...
	for "_i" from 1 to _num_waypoints do {
		...
		if(_waypoint_prev != "") then {
			_msg = ["STR_CL_PATROL_ARRIVE",[_waypoint_prev,(_waypoint select 0)]];
		} else {
			_msg = ["STR_CL_PATROL_MOVE",[(_waypoint select 0)]];
		};
		
		uiSleep random(10);
		
		_msg call wai_server_message;
		
		waitUntil{uiSleep 1;((_vehicle distance (_waypoint select 1) < 30) || !(alive leader _unitgroup) || (!canMove _vehicle))};
