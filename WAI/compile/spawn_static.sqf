...

{
	...
	if (!isNil "_mission") then {
		...
		((wai_mission_data select _mission) select 3) set [count ((wai_mission_data select _mission) select 3), [_unit,_static]]; // used for JIP invisible static gunner glitch
		((wai_mission_data select _mission) select 4) set [count ((wai_mission_data select _mission) select 4), _static];  // added for vehicle monitor
		...
		(wai_static_data select 2) set [count (wai_static_data select 2), _static];  // added for vehicle monitor
		(wai_static_data select 3) set [count (wai_static_data select 3), [_unit,_static]]; // used for JIP invisible static gunner glitch
	};
