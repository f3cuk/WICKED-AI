if(isServer) then {
	...
	[_mission,_position,"Hard","Captured Transport","MainBandit",true] call mission_init;

	diag_log 		format["WAI: [Mission:[Bandit] Captured Transport]: Starting... %1",_position];
	...
	[[
		...
		[(_position select 0) + 10, (_position select 1) - 10, 0]
	],"I_HMG_01_high_weapon_F","Hard","Hero","Hero",0,2,"Random","Random",_mission] call spawn_static;
	
	//Spawn vehicles
	_vehclass 		= "I_Heli_Transport_02_F";
	_vehicle		= [_vehclass,_position,_mission] call custom_publish;
	
	if(debug_mode) then {
		diag_log format["WAI: [Bandit] captured_mv22 spawned a Transport Heli at %1", _position];
	};
